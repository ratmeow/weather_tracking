...
from weather.users.schemas import UserDTO, UserRegisterRequest
...
class UserDAO:
    ...
    async def add(self, user_data: UserRegisterRequest) -> UserORM:
    ...
    async def get_by_login(self, login: str) -> Optional[UserORM]:
        query = select(UserORM).filter(UserORM.login == login)
        result = await self.db_session.execute(query)
        user = result.unique().scalar_one_or_none()
        return user

    async def add_session(self, user_id: int) -> UserSessionORM:
    ...
    async def get_session_by_id(self, session_id: str) -> Optional[UserSessionORM]:
        user_session: UserSessionORM = await self.db_session.get(UserSessionORM, session_id)  # pycharm bug
        return user_session

    async def update_session(self, user_session: UserSessionORM,
                             new_id: bool = False):
        if new_id:
    ...
    async def get_session_by_user_id(self, user_id: int) -> Optional[UserSessionORM]:
